...
from .cost import calculate, interactive, print_results, load_rework_data, generate_warnings, detect_repo_info
...
def cmd_full(args):
    ...
    rework_results = detect_rework(commits, args.window)
    print_rework_report(rework_results, args.window)
    ...
    if args.html:
        ...
        team = args.team if args.team is not None else detected_name
        repo_url = args.repo_url if args.repo_url is not None else detected_url
        warnings = generate_warnings(results, rework_results)
        html = generate_html(results, team, warnings, repo_url)
        with open(args.html, "w") as f:
            f.write(html)
        print(f"  HTML report written to {args.html}")
